...
function persistCareer() {
  if (!fighter) return;
  saveCareerState({ fighter, trainingLog, latestOffers, latestFightResult });
}
...
function renderCreateFighter() {
  app.innerHTML = `
    ...
    <div class='card'>
      ...
      <p class='small'>Includes fighter creation, training, matchmaking, fight simulation, and career history systems.</p>
    </div>
  `;
...
function renderMatchmakingCard() {
  return `
    <section class='card full'>
      <h2>Phase 4-5: Matchmaking + Fight Simulation</h2>
      <p class='small'>Review each offer, then accept one to simulate a 3-round bout and apply the result to your career.</p>
      ...
      <div class='grid'>
        ${latestOffers.map((opponent, index) => `
          <div class='card'>
            ...
            <p class='small'>Risk/reward: win pay about $${estimateWinPay(opponent)}, loss pay $1000. Better-ranked opponents can boost your career faster.</p>
            <button class='accept-fight' data-offer-index='${index}'>Accept Fight</button>
          </div>
        `).join("")}
      </div>
    </section>
...
function renderFightResultCard() {
  if (!latestFightResult) {
    return `
      <section class='card full'>
        <h2>Fight Result</h2>
        <p class='small'>No fight completed yet. Accept a fight offer to generate the first result.</p>
      </section>
    `;
  }
...
function renderOverview() {
  app.innerHTML = `
    ...
    <div class='grid'>
      ...
      <section class='card full'>
        <h2>Career Flow</h2>
        <p class='small'>Create a fighter, train week by week, accept fights, review results, and build a persistent career history.</p>
