...
import { saveCareerState, loadCareerState, clearCareerState } from "./systems/persistence";
...
if (loadedState) {
  fighter = loadedState.fighter ?? null;
  offers = loadedState.offers ?? [];
...
function persist() {
  saveCareerState({ fighter, offers, logs });
}
...
function render() {
  ...
  app.innerHTML = `<h1>MMA Career Manager MVP</h1><div class='grid'>
    ...
    <section class='card'><h2>Training</h2>
      <div class='row'><button id='save-game'>Save Career</button><button id='clear-save'>Clear Save</button></div>
  ...
  document.getElementById("refresh-offers").onclick = () => { offers = getFightOffers(fighter); persist(); render(); };

  document.getElementById("save-game").onclick = () => {
    const ok = saveCareerState({ fighter, offers, logs });
    logs.unshift(ok ? "Career saved." : "Save failed.");
    render();
  };

  document.getElementById("clear-save").onclick = () => {
    clearCareerState();
    logs.unshift("Saved career data cleared from browser storage.");
    render();
  };
